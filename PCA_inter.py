...
def get_args():
	...
	parser.add_argument('--output', default=None, help='output directory for SV txt files')
...
for i in range(len(sub_region_50kb)):
	...
	if sv_subregion.shape[0] < 10 or sv_subregion.shape[1] < 10 :
		continue
	out_file = os.path.join(save_path,f"{chr1}_{chr2}_{start1 + row_index}_{start2 + col_index}.txt")
	
	np.savetxt(out_file, sv_subregion, fmt="%d", delimiter=" ")
