...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../../'))) #把项目上层的某个目录加入Python模块搜索路径，便于导入工程内部模块。这里把当前文件上上级目录加入路径。
...
def draw(image, labels, boxes, scores, thrh=0.5):
    draw = ImageDraw.Draw(image)
    font = ImageFont.load_default()  # 可替换为更高质量的字体文件路径
    labels, boxes, scores = labels[scores > thrh], boxes[scores > thrh], scores[scores > thrh]     #按阈值过滤结果
